-- Add access_overrides column to ai_templates table if it doesn't exist
-- This migration adds a JSONB column to store access override settings similar to content_templates
...
BEGIN
    IF NOT EXISTS (
        ...
        AND table_name = 'ai_templates' 
        AND column_name = 'access_overrides'
    ) THEN
        -- Add the column to the table
        ALTER TABLE access.ai_templates 
        ADD COLUMN access_overrides JSONB DEFAULT '{}';
        
        -- Add comment explaining the purpose of the column
        COMMENT ON COLUMN access.ai_templates.access_overrides IS 'Stores JSON configuration for access overrides, similar to content_templates';
    END IF;
END $$;

-- Update the apply_templates_on_access_grant function to handle access_overrides for AI templates
CREATE OR REPLACE FUNCTION access.apply_templates_on_access_grant()
RETURNS TRIGGER AS $$
DECLARE
    template_record RECORD;
BEGIN
    -- Apply content templates if they exist
    IF TG_TABLE_NAME = 'user_access' AND NEW.type = 'content' THEN
        -- Check if there are any templates for this content
        FOR template_record IN 
            SELECT * FROM access.content_templates 
            WHERE content_id = NEW.target_id
        LOOP
            -- Apply the template's access_overrides to the new access record
            NEW.access_overrides = template_record.access_overrides;
            
            -- Grant access to any bundle variations associated with this template
            PERFORM access.grant_user_access(
                NEW.user_id, 
                bv.id, 
                'content'::access.access_type, 
                NEW.granted_at, 
                NEW.access_starts_at,
                NEW.access_overrides
            )
            FROM access.bundle_variations bv
            JOIN access.content_templates ct ON bv.template_id = ct.id
            WHERE ct.content_id = NEW.target_id;
        END LOOP;
    END IF;
    
    -- Apply AI templates if they exist
    IF TG_TABLE_NAME = 'user_access' AND NEW.type = 'ai' THEN
        -- Check if there are any templates for this AI tool
        FOR template_record IN 
        ...
        LOOP
            ...
            PERFORM access.grant_user_access(
                NEW.user_id, 
                bv.id, 
                'ai'::access.access_type, 
                NEW.granted_at, 
                NEW.access_starts_at,
                NEW.access_overrides
            )
            FROM access.bundle_variations bv
            JOIN access.ai_templates at ON bv.template_id = at.id
            WHERE at.target_id = NEW.target_id;
        END LOOP;
    END IF;
...
COMMENT ON FUNCTION access.apply_templates_on_access_grant() IS 'Trigger function that applies templates when access is granted, now supporting access_overrides for both content and AI templates';

-- Update the save_access_bundle function to handle access_overrides for AI templates
CREATE OR REPLACE FUNCTION public.save_access_bundle(p_bundle JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
        ...
        IF v_variation_id IS NOT NULL AND v_variation->'templates' IS NOT NULL AND jsonb_array_length(v_variation->'templates') > 0 THEN
          ...
          FOR i IN 0..jsonb_array_length(v_variation->'templates')-1 LOOP
            DECLARE
              ...
              v_access_overrides JSONB := COALESCE(v_template->'access_overrides', '{}'::JSONB);
            BEGIN
              ...
              IF v_template_type = 'content' THEN
                ...
                v_templates_array := v_templates_array || jsonb_build_object(
                  ...
                  'contentId', v_content_id,
                  'contentName', v_template->>'contentName',
                  'access_overrides', v_access_overrides
                );
                ...
                IF v_ai_level = 'category' THEN
                  ...
                  IF v_template_id IS NOT NULL AND v_template_id != '' THEN
                    ...
                    IF v_ai_template_exists THEN
                      ...
                      -- Update access_overrides if template exists
                      UPDATE access.ai_templates
                      SET access_overrides = v_access_overrides
                      WHERE id = v_ai_template_id;
                    ELSE
...
-- Add comment explaining the function
COMMENT ON FUNCTION public.save_access_bundle(JSONB) IS 'Saves an access bundle with variations and templates, now supporting access_overrides for both content and AI templates'; 