-- Create get_access_lists_organized in access schema
...
-- Create get_list_members in access schema
...
-- Create public wrapper functions that call the access schema functions
CREATE OR REPLACE FUNCTION public.get_access_lists_organized(
    p_list_type TEXT[] DEFAULT NULL
) RETURNS JSONB AS $$
BEGIN
    RETURN access.get_access_lists_organized(p_list_type);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    RETURN QUERY SELECT * FROM access.get_list_members(p_list_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute permissions
GRANT EXECUTE ON FUNCTION access.get_access_lists_organized(TEXT[]) TO authenticated;
GRANT EXECUTE ON FUNCTION access.get_list_members(UUID) TO authenticated;
